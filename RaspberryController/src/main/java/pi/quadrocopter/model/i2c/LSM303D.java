...
public class LSM303D extends QI2CDevice {
    ...
    @Getter
    private final Vector3_16bit accel = new Vector3_16bit();
    ...
    @Getter
    private final Vector3_16bit mag = new Vector3_16bit();
    ...
    private void readMag() {
        try {
            ...
            mag.setY(yhm, ylm);
            mag.setZ(zhm, zlm);
            mag.x -= MagnetometerCalibration.offset.x;
            mag.y -= MagnetometerCalibration.offset.y;
            mag.z -= MagnetometerCalibration.offset.z;
        } catch (IOException e) {
            System.out.println(e.getMessage());
        }
    }
