...
public class NRF24 {
    ...
    private static class NRF24Pins {
    ...
    public NRF24(){
        try{
            spi = SpiFactory.getInstance(NRF24Pins.channel,SpiDevice.DEFAULT_SPI_SPEED,SpiDevice.DEFAULT_SPI_MODE);
        }catch(IOException e){
            e.printStackTrace();
        }
        outputPin=gpio.provisionDigitalOutputPin(NRF24Pins.ce,PinState.LOW);
        wide_band=true;
        p_variant=false;
        payload_size=32;
        ack_payload_available=false;
        dynamic_payloads_enabled=false;
        Arrays.fill(pipe0_reading_address, (byte) 0);
    }
